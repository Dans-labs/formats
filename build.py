            ...
            for (item, itemInfo) in sorted(fileFormats.items(), key=sortKey2):
                ...
                theseRelated = ", ".join(
                    ...
                    for x in sorted(
                        set(
                            chain.from_iterable(
                                filesFromType[dtype] for dtype in typesFromFile[item]
                            )
                        ),
                ...
                row = " | ".join(
                    (display, thisPreferred, theseExtensions, theseRelated, theseTypes)
                )
                text.append(f"{row}\n")
            return "".join(text)
...
"""
        ...
        def writeIndex():
            ...
            text = text.replace("[[overview]]", makeOverview()).replace(
                "[[preferred]]", makePreferred()
            )
            path = f"{DOCS}/{INDEX}"
...
"""
        ...
        def writeFileFormats():
            for (item, itemInfo) in fileFormats.items():
                ...
                theseWikis = ", ".join(
                    f"[`{x}`]({wikiloc}/{x})" for x in itemInfo["wikipedia"]
                )
                theseLinks = "".join(f"*   [`{x}`]({x})\n" for x in itemInfo["links"])
...
"""
        ...
        def writeExtensions():
            for (item, itemInfo) in extensions.items():
                ...
                exts = sorted(
                    set(chain.from_iterable(extsFromFile[x] for x in theseF)),
                ...
                if "[[urls]]" in line:
                    ...
                    for (item, itemInfo) in sorted(dataTypes.items(), key=sortKey2):
                        ...
                        for subitem in sorted(
                            ...
                            text.append(
                                f"      - {subdisplay}: 'fileFormats/{subitem}.md'\n"
                            )
                else:
                    text.append(line)
